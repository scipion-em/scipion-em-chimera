...
def getEnvDictionary(prot):
    ...
    # Get the values
    _chimeraPdbTemplateFileName = os.path.abspath(
        prot._getExtraPath(chimeraPdbTemplateFileName))
    _chimeraMapTemplateFileName = os.path.abspath(
        prot._getExtraPath(chimeraMapTemplateFileName))
    _sessionFile = os.path.abspath(prot._getExtraPath(sessionFile))
    protId = prot.getObjId()

    # Populate the dictionary and return
    envDict = {
        CHIMERA_PDB_TEMPLATE_FILE_NAME: _chimeraPdbTemplateFileName % protId,
